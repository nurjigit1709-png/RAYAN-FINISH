...
export default function ChatPage() {
  ...
  const { data: messages = [] } = useListMessages(
    { userId: selectedUserId! },
    {
      query: {
        ...
        queryKey: getListMessagesQueryKey({ userId: selectedUserId! }),
      },
    },
  ) as any;

  const { data: allFiles = [] } = useListFiles({}) as any;
  const { data: allUsers = [] } = useListUsers() as any;

  const sendMessage = useSendMessage();

  const conversationMap = new Map(
    (conversations as any[]).map((c: any) => [c.userId, c]),
  );

  const filteredConvs = (allUsers as any[])
    .filter((u: any) => u.id !== user?.id && !u.isBlocked)
    .map((u: any) => {
      const conv = conversationMap.get(u.id) as any;
      return {
        userId: u.id,
        userName: u.name,
        userRole: u.role,
        ...
        lastFileId: conv?.lastFileId || null,
      };
    })
    .filter((c: any) =>
  ...
  const selectedConv =
    filteredConvs.find((c: any) => c.userId === selectedUserId) ||
    (conversations as any[]).find((c: any) => c.userId === selectedUserId);

  const filteredFiles = (allFiles as any[])
    .filter((f: any) => f.name.toLowerCase().includes(fileSearch.toLowerCase()))
    .slice(0, 20);
    ...
    >
      <div className="px-3 py-3 border-b border-border flex items-center gap-3 flex-shrink-0">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {selectedConv && getRoleBadge(selectedConv.userRole)}
          </p>
        </div>
      </div>
                  ...
                  {msg.fileId && (
                    ...
                    >
                      <span className="text-lg flex-shrink-0">
                        {getFileIconEmoji(msg.fileMimeType || "")}
                      </span>
      ...
      {attachFile && (
        <div className="mx-3 mb-2 flex items-center gap-2 bg-accent/50 border border-border rounded-lg px-3 py-2 flex-shrink-0">
          ...
          <p className="text-sm flex-1 truncate text-foreground">
            {attachFile.name}
          </p>
          <button
